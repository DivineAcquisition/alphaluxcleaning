-- Security fixes: tighten RLS and add safe rate limiting

-- 1) Drop permissive policies that allow anonymous access
...
-- 2) Recreate stricter policies
CREATE POLICY "Authenticated users can insert commercial estimates" ON public.commercial_estimates
FOR INSERT
WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can select their commercial estimates" ON public.commercial_estimates
FOR SELECT
...
-- 3) Enhanced admin verification (no header dependency)
...
-- 4) Safe rate limiting for auth attempts
...
-- Only system (service role) can manage
CREATE POLICY IF NOT EXISTS "System manages rate limits" ON public.auth_rate_limits
...
DECLARE
  ...
  v_new_count integer;
BEGIN
  -- Try to insert a new bucket row, otherwise increment
  ...
  -- Block after 5 attempts in the hour
