...
async function executeServiceLifecycleWorkflow(params: any, token: string, locationId: string, supabase: any) {
  ...
  const workflows = {
    ...
    'service_completed': {
      actions: [
        { type: 'send_sms', message: `Your ${serviceType} is complete! Thank you for choosing Bay Area Cleaning Pros. How did we do?` },
...
async function executeAppointmentReminders(params: any, token: string, locationId: string, supabase: any) {
  ...
  const reminders = {
    ...
    '30_minute': {
      message: `We're on our way! Your cleaning team will arrive in 30 minutes. Thank you for choosing Bay Area Cleaning Pros! 🚗`,
      urgency: 'urgent'
    }
  };
...
async function sendAutomatedEmail(customerEmail: string, subject: string, template: string, token: string, locationId: string) {
  ...
  const emailTemplates = {
    ...
    'service_completion': `Thank you for choosing Bay Area Cleaning Pros! We hope you love your clean space. Please rate our service!`
  };
